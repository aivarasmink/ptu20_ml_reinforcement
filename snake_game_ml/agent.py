...
class Agent:
    def __init__(self):
        # Initialization of agent parameters and attributes
        self.n_games = 0
        self.epsilon = 0  # Exploration rate
        self.gamma = 0.9  # Discount rate
        self.memory = deque(maxlen=MAX_MEMORY)  # Replay memory
        self.model = Linear_QNet(11, 256, 3)  # Q-network model
        self.trainer = QTrainer(self.model, lr=LR, gamma=self.gamma)  # Q-trainer for training the model
    ...
    def remember(self, state, action, reward, next_state, done):
        # Stores the experience tuple (state, action, reward, next_state, done) into the agent's memory buffer
        self.memory.append((state, action, reward, next_state, done))

    def train_long_memory(self):
        # Trains the neural network model using experiences from the agent's memory buffer
    ...
    def train_short_memory(self, state, action, reward, next_state, done):
        # Trains the neural network model with a single experience tuple
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 200) < self.epsilon:
            move = random.randint(0, 2)  # Explore randomly
        else:
